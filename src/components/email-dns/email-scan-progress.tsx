...
import { CheckCircle2, Loader2, Globe, Shield, Key, Lock, AlertTriangle } from "lucide-react";
import { Card } from "@/components/ui/card";
import type { EmailScanProgress as EmailScanProgressType } from "@/types/email-scan";
import { cn } from "@/lib/utils";

const ALL_CHECKS = [
  { key: "dns", label: "DNS & MX Records", description: "Checking mail server configuration", icon: Globe },
  { key: "spf", label: "SPF Record", description: "Validating sender policy", icon: Shield },
  { key: "dkim", label: "DKIM Keys", description: "Checking email signatures", icon: Key },
  { key: "dmarc", label: "DMARC Policy", description: "Verifying email authentication policy", icon: Lock },
  { key: "blacklist", label: "Blacklist Check", description: "Scanning reputation databases", icon: AlertTriangle },
];

interface Props {
  progress: EmailScanProgressType[];
}

export function EmailScanProgress({ progress }: Props) {
      ...
      <div className="mb-5 flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="mt-0.5 text-[0.8125rem] text-[var(--text-tertiary)]">
            Running {ALL_CHECKS.length} email checks
          </p>
        </div>
      </div>
